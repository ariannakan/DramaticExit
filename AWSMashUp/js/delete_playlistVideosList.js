...
function refreshPlaylistVideosList() {
   ...
   xhr.open("GET", list_playlist_videos_url, true);
   xhr.send();
...
function processListResponse(result) {
  ...
  var vidList = document.getElementById('playlistVideosList');
  
  var output = "";
  for (var i = 0; i < js.list.length; i++) {
    var playlistVideosJson = js.list[i];
    console.log(playlistVideosJson);
    
    var playlistName = playlistVideoJson["playlistName"];
    var videoID = playlistVideosJson["videoID"];
